/*
    File: Varsity_flight_test/vision_position_and_speed_estimate.cpp

    Description:
      This script performs camera depth estimation and velocity estimation using
      Hailo and OpenCV. It communicates with an ArduPilot-based flight controller
      via MAVLink by sending both VISION_SPEED_ESTIMATE (ID 103) and VISION_POSITION_ESTIMATE (ID 102)
      messages each processing loop. A separate thread reads and prints MAVLink telemetry.
      
      Note: This example integrates the computed speeds over time to generate a very basic
      position estimate. Orientation values are set to zero.
      
    Build requirements:
      - Hailo runtime libraries and headers
      - OpenCV libraries
      - MAVLink headers (e.g. mavlink/v2.0/common/mavlink.h)
...
const unsigned char GYRO_Y_HIGH  = 0x25;
const unsigned char GYRO_Y_LOW   = 0x26;
const unsigned char GYRO_X_HIGH  = 0x27;
const unsigned char GYRO_X_LOW   = 0x28;
const unsigned char GYRO_Z_HIGH  = 0x29;
const unsigned char GYRO_Z_LOW   = 0x2A;
    ...
    while (mavlinkRunning.load()) {
        {
            ...
            std::cout << "  Heartbeat: " << (heartbeatReceived ? "RECEIVED" : "NOT RECEIVED") << std::endl;
...
//-------------------------------------------------------------
// MAVLink Message Sending Functions
//-------------------------------------------------------------
//
// Sends VISION_SPEED_ESTIMATE (ID 103) with additional covariance info.
// If you don't have a valid covariance, a dummy array of zeros is provided.
void sendVisionSpeedEstimate(int fd, float vx, float vy, float vz)
{
    ...
    float dummy_cov[9] = {0.0f}; // 3x3 covariance (dummy)
    uint8_t cov_len = 9;
    mavlink_msg_vision_speed_estimate_pack(system_id, component_id, &msg,
                                             usec, vx, vy, -3*vz, dummy_cov, cov_len);
...
// Sends VISION_POSITION_ESTIMATE (ID 102) with position and orientation (r, p, y).
// In this example, orientation is set to zero and a dummy covariance array is provided.
void sendVisionPositionEstimate(int fd, float x, float y, float z, float roll, float pitch, float yaw)
{
    ...
    // Provide a dummy 21-element covariance array (expected by VISION_POSITION_ESTIMATE).
    float dummy_cov[21] = {0.0f}; // 21 floats, which is 84 bytes
    uint8_t cov_length = 21;      // Length of the covariance array

    // Pack the message with the covariance parameters.
    mavlink_msg_vision_position_estimate_pack(system_id, component_id, &msg,
                                                usec, x, y, z, roll, pitch, yaw,
                                                dummy_cov, cov_length);

    uint8_t buffer[MAVLINK_MAX_PACKET_LEN];
    int len = mavlink_msg_to_send_buffer(buffer, &msg);
    if (write(fd, buffer, len) < 0) {
        std::cerr << "Failed to send VISION_POSITION_ESTIMATE message." << std::endl;
    }
}
...
int main(int argc, char** argv)
{
    ...
    // For simple position integration.
    auto pos_last_time = std::chrono::steady_clock::now();
    static double pos_x = 0.0, pos_y = 0.0, pos_z = 0.0;
    ...
    while (true) {
        ...
        // Compute Predicted Rotational Flow using gyro data.
        {
        ...
        // Convert cm/s speeds back to m/s.
        float vx_mps = static_cast<float>(Vx) / 100.0f;
        float vy_mps = static_cast<float>(Vy) / 100.0f;
        float vz_mps = static_cast<float>(Vz) / 100.0f;

        // Send the vision speed estimate.
        sendVisionSpeedEstimate(mavlinkFd, vx_mps, vy_mps, vz_mps);

        // Integrate speeds over time to generate a simple position estimate.
        auto pos_current_time = std::chrono::steady_clock::now();
        double dt = std::chrono::duration<double>(pos_current_time - pos_last_time).count();
        pos_last_time = pos_current_time;
        pos_x += vx_mps * dt;
        pos_y += vy_mps * dt;
        pos_z += vz_mps * dt;

        // Send the vision position estimate with orientation set to zero.
        sendVisionPositionEstimate(mavlinkFd, static_cast<float>(pos_x),
