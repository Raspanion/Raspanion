                           ...
                           cv::Mat &calib_depth)
{
    ...
    // Compute calibrated depth (in meters).
    ...
    // Normalize depth map using min/max from ROI.
    ...
    // Overlay min and max depth values.
...
//-------------------------------------------------------------
// Gyro thread globals and functions
//-------------------------------------------------------------
// I2C address and register definitions for ICM-42688-P
const int ICM42688_ADDR = 0x69;  // Adjust if necessary
const unsigned char WHO_AM_I_ICM = 0x75;
const unsigned char PWR_MGMT_0   = 0x4E;
const unsigned char GYRO_CONFIG0 = 0x4F;
const unsigned char GYRO_DATA_X0 = 0x25;  // 6 bytes: X0, X1, Y0, Y1, Z0, Z1

std::mutex gyro_mutex;
// Global mapped gyro values and raw gyro values.
double g_gyro_x = 0.0, g_gyro_y = 0.0, g_gyro_z = 0.0;
std::atomic<int> g_raw_x{0}, g_raw_y{0}, g_raw_z{0};
std::atomic<bool> g_gyro_running{true};

void gyro_thread_func()
{
    const char *i2c_device = "/dev/i2c-1";
    int fd = open(i2c_device, O_RDWR);
    if (fd < 0)
    {
        std::cerr << "[Gyro Thread] Failed to open I2C device " << i2c_device << std::endl;
        return;
    }
    ...
    usleep(50000); // Allow stabilization
    ...
    while (g_gyro_running.load())
    {
        ...
        int16_t gx_raw = (data[1] << 8) | data[0];
        int16_t gy_raw = (data[3] << 8) | data[2];
        int16_t gz_raw = (data[5] << 8) | data[4];

        {
            std::lock_guard<std::mutex> lock(gyro_mutex);
            // Update raw values.
            g_raw_x.store(gx_raw);
            g_raw_y.store(gy_raw);
            g_raw_z.store(gz_raw);
            // Swap the labels as determined:
            //   Use raw Y for roll (g_gyro_x),
            //   raw X for pitch (g_gyro_y),
            //   raw Z for yaw (g_gyro_z).
            g_gyro_x = - gy_raw * GYRO_RAW_TO_DPS * DPS_TO_RAD;
            // g_gyro_x = - gy_raw * 0.0;
            g_gyro_y =  gx_raw * GYRO_RAW_TO_DPS * DPS_TO_RAD;
            // g_gyro_y = - gx_raw * 0.0;
            g_gyro_z =  gz_raw * GYRO_RAW_TO_DPS * DPS_TO_RAD;
            // g_gyro_z = - gz_raw * 0.0;
        }
        usleep(5000); // 5 ms
    }
...
int main(int argc, char** argv)
{
    ...
    // Start the gyro thread.
    std::thread gyro_thread(gyro_thread_func);
    ...
    while (true)
    {
        ...
        // --- Optical Flow Computation (downsampled grayscale image) ---
        cv::Mat gray;
        ...
        // --- Compute LS Fit for Angular Rate Estimation ---
        int step = 10;
        ...
        // Using our original definitions:
        double Vz = omega_x * avg_depth;
        double Vy = omega_y * avg_depth;
        
        // For Vx, use divergence method.
        ...
        double Vx = - (sum_weighted_div / area) / 2.0;
        
        // --- Compute Predicted Rotational Flow from Gyro ---
        cv::Mat predicted_flow(flow.size(), flow.type(), cv::Scalar(0,0));
        {
            std::lock_guard<std::mutex> lock(gyro_mutex);
            // Using the mapping:
            //   g_gyro_x: roll (from raw Y), 
            //   g_gyro_y: pitch (from raw X),
            //   g_gyro_z: yaw (from raw Z).
            // Predicted flow:
            //   u_pred = f_small * (g_gyro_z) + y_centered * (g_gyro_x)
            //   v_pred = -f_small * (g_gyro_y) - x_centered * (g_gyro_x)
            for (int y = 0; y < flow.rows; y += step)
            {
                for (int x = 0; x < flow.cols; x += step)
                {
                    double x_centered = x - cx_small;
                    double y_centered = y - cy_small;
                    float pred_u = static_cast<float>( f_small * g_gyro_z + y_centered * g_gyro_x );
                    float pred_v = static_cast<float>( -f_small * g_gyro_y - x_centered * g_gyro_x );
                    predicted_flow.at<cv::Point2f>(y, x) = cv::Point2f(pred_u, pred_v);
                }
            }
        }
        
        // --- Create Overlay Image ---
        // Start with a copy of the depth color image.
        cv::Mat overlay = depth_color.clone();
        
        // First, draw the predicted (blue) arrows.
        for (int y = 0; y < flow.rows; y += step)
        {
            for (int x = 0; x < flow.cols; x += step)
            {
                cv::Point pt1(x * 2, y * 2);
                cv::Point2f pred_fxy = predicted_flow.at<cv::Point2f>(y, x);
                cv::Point pt2_pred(cvRound(x * 2 + pred_fxy.x * 2), cvRound(y * 2 + pred_fxy.y * 2));
                cv::arrowedLine(overlay, pt1, pt2_pred, cv::Scalar(255, 0, 0), 3);
            }
        }
        
        // Then, draw the measured (green) optical flow arrows on top.
        for (int y = 0; y < flow.rows; y += step)
        {
            for (int x = 0; x < flow.cols; x += step)
            {
        ...
        cv::putText(overlay, oss_vx.str(), cv::Point(baseX, baseY),
        ...
        cv::putText(overlay, oss_vy.str(), cv::Point(baseX, baseY + 20),
                    cv::FONT_HERSHEY_SIMPLEX, speedFontScale, cv::Scalar(0,0,0), speedOutlineThickness);
        cv::putText(overlay, oss_vy.str(), cv::Point(baseX, baseY + 20),
                    cv::FONT_HERSHEY_SIMPLEX, speedFontScale, cv::Scalar(255,255,255), speedThickness);
        cv::putText(overlay, oss_vz.str(), cv::Point(baseX, baseY + 40),
                    cv::FONT_HERSHEY_SIMPLEX, speedFontScale, cv::Scalar(0,0,0), speedOutlineThickness);
        cv::putText(overlay, oss_vz.str(), cv::Point(baseX, baseY + 40),
                    cv::FONT_HERSHEY_SIMPLEX, speedFontScale, cv::Scalar(255,255,255), speedThickness);
